...
def toLogDecider (backend : Skelet1Backend) (cfg: List DeciderConfig) (quiet: Bool)
    (M: Machine l s): HaltM M Unit := do
  let res := toTableDecider backend cfg M
...
def classifyRecord (backend : Skelet1Backend) (cfg: List DeciderConfig) (M: Machine l s):
    Witness.Record × Witness.Outcome :=
  let (res, who) := decideWithProvenance backend cfg M
  let code := reprStr M
  match res with
  | .halts_prf n C _ =>
      let st := C.state.val
...
def emitClassify (backend : Skelet1Backend) (store: Witness.Store)
    (cfg: List DeciderConfig) (M: Machine l s):
    IO Witness.Outcome := do
  let (rec, outcome) := classifyRecord backend cfg M
  store.put rec
  return outcome
...
def emitClassifyChan (backend : Skelet1Backend)
    (chan: Std.CloseableChannel Witness.Record) (cfg: List DeciderConfig)
...
def runDecideCmd (backend : Skelet1Backend) (p: Parsed): IO UInt32 := do
...
def dummyCmd (_p : Parsed) : IO UInt32 := pure 0

unsafe def decideCmdSpec := `[Cli|
  decide VIA dummyCmd;
...
def auditLine (backend : Skelet1Backend)
    (resolveConfig : ℕ → ℕ → List DeciderConfig) (line : String) : IO Unit := do
  ...
  | .success _ ⟨l, s, M⟩ =>
      match firstDecision? backend (resolveConfig l s) M with
      | none => IO.println s!"{code} unknown"
      | some (decider, result) => IO.println s!"{code} {result} by {decider}"

def runAuditCmd (backend : Skelet1Backend) (p: Parsed): IO UInt32 := do
  ...
  for line in lines do
    auditLine backend resolveConfig line
  return 0

unsafe def auditCmdSpec := `[Cli|
  audit VIA dummyCmd;
...
unsafe def exportRec (backend : Skelet1Backend) (cfg: List DeciderConfig) (M: Machine l s)
      ...
      | .halts_prf n C _ =>
          ...
          if M.n_halting_trans > 1 then
            for M' in Quot.unquot (Busybeaver.next_machines M C.state C.tape.head).val do
              exportRec backend cfg M' emit

unsafe def runExportCmd (backend : Skelet1Backend) (p: Parsed): IO UInt32 := do
  ...
  IO.FS.withFile output IO.FS.Mode.write fun h => do
    Tqdm.withProgressBar (cfg := {}) fun pb => do
      ...
      if l = 0 then
        ...
        exportRec backend cfg (Busybeaver.BBCompute.m1RB l s) emit
      else
        exportRec backend cfg (Busybeaver.BBCompute.m0RB l s) emit
        exportRec backend cfg (Busybeaver.BBCompute.m1RB l s) emit
  return 0

unsafe def exportCmdSpec := `[Cli|
  enumerate VIA dummyCmd;
...
unsafe def computeCmd (backend : Skelet1Backend) (p: Parsed): IO UInt32 := do
  ...
  if p.hasFlag "trusted" then
    ...
    if l = 0 then
      ...
      | none =>
          ...
          let res ← Witness.walkRootsPar (l := l) (s := s)
            (fun M => trustedClassify store (emitClassify backend store cfg) M)
    ...
    let dec := toLogDecider backend cfg (p.hasFlag "quiet")
    if hl: l = 0 then
      ...
      let res ← do
        try
          Witness.walkRootsPar (l := l) (s := s)
            (fun M => emitClassifyChan backend chan cfg M)
        finally
...
unsafe def withRun (cmd : Cli.Cmd) (run : Parsed → IO UInt32) : Cli.Cmd :=
  match cmd with
  | .init «meta» _ subCmds extension? => .init «meta» run subCmds extension?

unsafe def subcommandWithBackend (backend : Skelet1Backend) (cmd : Cli.Cmd) : Cli.Cmd :=
  match cmd.meta.name with
  | "decide" => withRun cmd (runDecideCmd backend)
  | "audit" => withRun cmd (runAuditCmd backend)
  | "explore" => withRun cmd runExploreCmd
  | "enumerate" => withRun cmd (runExportCmd backend)
  | _ => cmd

unsafe def mainCmd (backend : Skelet1Backend) : Cli.Cmd :=
  match mainCmdSpec with
  | .init «meta» _ subCmds extension? =>
      .init «meta» (computeCmd backend)
        (subCmds.map (subcommandWithBackend backend)) extension?

unsafe def main (backend : Skelet1Backend) (args: List String): IO UInt32 := do
  (mainCmd backend).validate args
